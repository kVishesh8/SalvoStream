...
const tracker_js_1 = require("../health/tracker.js");
class ProwlarrClient {
     ...
     */
    async verifyFlareSolverrConnectivity() {
        if (!env_js_1.config.FLARESOLVERR_ENABLED) {
            this.logger.info("FlareSolverr integration is disabled (FLARESOLVERR_ENABLED=false).");
            return false;
        }
        const checkUrl = `${env_js_1.config.FLARESOLVERR_URL}/health`;
        this.logger.info({ url: env_js_1.config.FLARESOLVERR_URL }, "Checking FlareSolverr availability...");
        try {
            const response = await fetch(checkUrl, { method: "GET" });
            if (response.ok) {
                this.logger.info({ url: env_js_1.config.FLARESOLVERR_URL }, "FlareSolverr connection verified. Host is online.");
                return true;
            }
            else {
    ...
    async search(query, type) {
                    ...
                    if (idx && idx.enable && idx.protocol === "torrent" && idx.name) {
                        ...
                        if (idx.status !== undefined && idx.status !== null) {
                            ...
                            if (isExplicitFailure) {
                                ...
                                (0, sqlite_js_1.recordIndexerFailure)(idx.name, errorMsg);
        ...
        const trackerHealth = new tracker_js_1.TrackerHealthManager(this.logger);
        ...
        const searchPromises = activeIds.map(async (id) => {
            ...
            try {
                ...
                if (!response.ok) {
                    (0, sqlite_js_1.recordIndexerFailure)(trackerName, `HTTP Error ${response.status}`);
                ...
                if (!Array.isArray(results)) {
                    (0, sqlite_js_1.recordIndexerFailure)(trackerName, "Invalid JSON response (expected array)");
                    return [];
                }
                // Record successful query stats in SQLite database
                (0, sqlite_js_1.recordIndexerSuccess)(trackerName, trackerLatency);
                ...
                if (err.name === "AbortError") {
                    (0, sqlite_js_1.recordIndexerFailure)(trackerName, "Query Timeout (6000ms exceeded)");
                    this.logger.warn({ indexer: trackerName, latencyMs: trackerLatency }, "Tracker query timed out");
                }
                else {
                    (0, sqlite_js_1.recordIndexerFailure)(trackerName, err.message || "Network Error");
