 ...
 */
export function parseAndValidateInfoHash(magnetUrl?: string, directHash?: string): string | null {
 ...
 */
export function isSpamOrNSFW(title: string): boolean {
...
export function processAndRenderStreams(
  ...
  const seenTitleSizes = new Set<string>();
  ...
  for (const item of sortedResults) {
    ...
    if (item.size > 0) metaParts.push(formattedSize);
