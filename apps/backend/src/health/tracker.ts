import { getIndexerHealthStats } from "../db/sqlite.js";
...
export class TrackerHealthManager {
  private logger: any;
  ...
  getHealthyIndexerIds(prowlarrIndexers: ProwlarrIndexer[]): string | undefined {
    ...
    const healthStats = getIndexerHealthStats();
    ...
    // 3. Filter indexers
    const healthyIndexers = activeIndexers.filter(
      idx => !unhealthySet.has(idx.name.toLowerCase())
    );
